...
import axios from "axios"; // axios를 임포트하여 HTTP 요청을 처리합니다.
...
function Main() {
  ...
  const fetchVehicles = async () => {
    ...
    try {
      ...
      // axios를 사용하여 GET 요청
      const { data } = await axios.get(`http://localhost:3001/api/vehicles?${query}`);

      setVehicles((prevVehicles) => [...prevVehicles, ...data]); // 기존 차량 데이터에 추가
      setHasMore(data.length > 0); // 추가 데이터가 있는지 여부 설정
